...
import { 
  ...
  DollarSign, Heart, Share2, Play, Tag, Briefcase
} from 'lucide-react';
...
export default function App() {
  ...
  // Chart Data Preparation (Ops View)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg shadow-md transition-colors ${viewMode === 'exec' ? 'bg-slate-900 shadow-slate-300' : 'bg-indigo-600 shadow-indigo-200'}`}>
                {viewMode === 'exec' ? <Briefcase className="w-5 h-5 text-white" /> : <LayoutDashboard className="w-5 h-5 text-white" />}
              </div>
              <div>
                <h1 className="text-xl font-bold text-slate-900 tracking-tight leading-none">Tweely {viewMode === 'exec' ? 'Executive' : 'Ops Board'}</h1>
                <p className="text-xs text-slate-500 font-medium">{viewMode === 'exec' ? 'Strategic Overview & High-Level Metrics' : 'Official Store & Content Command Center'}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center bg-slate-100 rounded-lg p-1 border border-slate-200">
                 <button 
                   ...
                   className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wide transition-all ${viewMode === 'ops' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-800'}`}
                 >
                   Ops View
                 </button>
                 <button 
                   onClick={() => setViewMode('exec')}
                   className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wide transition-all ${viewMode === 'exec' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-800'}`}
                 >
                   Executive
                 </button>
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {viewMode === 'exec' ? (
          <ExecutiveDashboard 
            ...
            onShopeeUpload={handleShopeeUpload}
            onTiktokUpload={handleTiktokUpload}
          />
        ) : (
          /* OPS VIEW */
          <>
